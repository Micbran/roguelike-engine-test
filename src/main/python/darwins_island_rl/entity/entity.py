...
class Entity:
    ...
    """
    def __init__(self, x, y, char, color, name, blocks=False, combat=None, ai=None, render_order=RenderOrder.CORPSE):
        self.x = x
        self.y = y
        self.char = char
        self.color = color
    ...
    def move_astar(self, target, entities, game_map):
        ...
        for y1 in range(game_map.height):
            for x1 in range(game_map.width):
                tcod.map_set_properties(fov, x1, y1, not game_map.tiles[x1][y1].sight_block,
                                        not game_map.tiles[x1][y1].move_block)
